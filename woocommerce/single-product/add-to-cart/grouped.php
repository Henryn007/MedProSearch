...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	<!-- <table cellspacing="0" class="woocommerce-grouped-product-list group_table"> -->
		<tbody>
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				echo '<div id="product-' . esc_attr( $grouped_product_child->get_id() ) . '" class="actions woocommerce-grouped-product-list-item ' . esc_attr( implode( ' ', wc_get_product_class( '', $grouped_product_child ) ) ) . '">';
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					echo '<div class="quantity style2 me-4 woocommerce-grouped-product-list-item__' . esc_attr( $column_id ) . '">' . apply_filters( 'woocommerce_grouped_product_list_column_' . $column_id, $value, $grouped_product_child ) . '</div>'; // WPCS: XSS ok.
	...
	<?php if ( $quantites_required ) : ?>
        ...
        <div class="actions-btn">
		    ...
		    <button type="submit" class="single_add_to_cart_button vs-btn style2"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
		    <?php
				// Wishlist Button
