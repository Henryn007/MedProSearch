...
<div class="vs-comment-area vs-comments-layout1">
    <h3 class="h2 inner-title1 mb-lg-4 pb-lg-2">
        <?php printf( _nx( '1 Comment', '%1$s Comments', get_comments_number(), 'comments title', 'medprosearch' ), number_format_i18n( get_comments_number() ) ); ?>
    </h3>
        ...
        <?php
            the_comments_navigation();
                wp_list_comments( array(
                    ...
                    'short_ping'  => true,
                    'avatar_size' => 100,
                    'callback'    => 'medprosearch_comment_callback'
                ) );
            the_comments_navigation();
        ?>
    </ul>
...
<?php
	...
	$fields =  array(
	  'author'  => '<div class="row"><div class="col-md-6 form-group"><input class="form-control style3" type="text" name="author" placeholder="'. esc_attr__( 'Enter Your Name *', 'medprosearch' ) .'" value="'. esc_attr( $commenter['comment_author'] ).'" '.esc_attr( $aria_req ).'></div>',
	...
	$args = array(
        ...
        'logged_in_as'          => '<p class="logged-in-as mb-4 pb-2">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>','medprosearch' ), admin_url( 'profile.php' ), esc_attr( $user_identity ), wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
        'class_submit'          => 'vs-btn style2',
        'submit_field'          => '<div class="row"><div class="col-12 form-group pt-xl-20 mb-0">%1$s %2$s</div></div>',
    	'submit_button'         => '<button type="submit" name="%1$s" id="%2$s" class="%3$s">'.esc_html__('Post Comment','medprosearch').'<i class="fas fa-chevron-right"></i></button>',
    	'fields'                => $fields,
	);
